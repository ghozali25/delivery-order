        ...
        <div class='flex justify-between items-center gap-2'>
            ...
            <div class='flex items-center gap-2'>
                ...
                <div x-data='{showModal:false}'>
                    ...
                    <livewire:lookup.destination-area.form.add key="add" />
                </div>
            </div>
        </div>
            ...
            <table class='w-[100%]'>
                ...
                <tbody>
                    @foreach ($this->destinationAreas as $destinationArea)
                        <tr wire:key='row-{{ $destinationArea->id }}' class='hover:bg-stone-100 border-y'>
                            <td class='p-2'>
                                <div class='flex items-center'>
                                    <div x-data='{showModal:false}'>
                                        <button type='button' x-on:click='showModal=true' class='btn-ghost-icon-stone'><i class='bi-pencil-square'></i></button>
                                        <livewire:lookup.destination-area.form.edit :destinationArea="$destinationArea->id" :key="'edit-' . $destinationArea->id" />
                                    </div>
                                    <div x-data='{showModal:false}'>
                                        <button type='button' x-on:click='showModal=true' class='btn-ghost-icon-red'><i class='bi-trash'></i></button>
                                        <livewire:lookup.destination-area.form.delete :destinationArea="$destinationArea->id" :key="'delete-' . $destinationArea->id" />
                                    </div>
                                </div>
                            </td>
                            <td class='p-2'>{{ $destinationArea->abbreviation ?? '-' }}</td>
                            <td class='p-2'>{{ $destinationArea->name ?? '-' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
