        ...
        <div class='overflow-auto'>
            <div class='py-5 px-10 flex flex-col gap-5'>
                <div class='input-container'>
                    ...
                    <select wire:model.live='id_role' class='input-select @error('id_role') input-state-error @enderror input-state-normal'>
                        <option value=''></option>
                ...
                <div class='input-container'>
                    <div class='flex justify-between items-center'>
                        <label class='font-semibold'>KTP</label>
                        @error ('ktp')
                            <div class='group/error relative flex items-center gap-2'>
                                <p class='error-message'>{{ $message }}</p>
                                <i class='bi-info-circle text-red-500'></i>
                    ...
                    <input wire:model.live='ktp' type='text' spellcheck='false' class='input-general @error('ktp') input-state-error @enderror input-state-normal'>
                </div>
                <div class='input-container'>
                    <div class='flex justify-between items-center'>
                ...
                <div class='input-container'>
                    <div class='flex justify-between items-center'>
                        <label class='font-semibold'>Aktif</label>
                        @error ('is_active')
                            <div class='group/error relative flex items-center gap-2'>
                                <p class='error-message'>{{ $message }}</p>
                                <i class='bi-info-circle text-red-500'></i>
                    ...
                    <label class='cursor-pointer {{ $is_active ? 'btn-stone' : 'btn-ghost-stone' }}'>
                        <p>{{ $is_active ? 'Ya' : 'Tidak' }}</p>
                        <input wire:model.live='is_active' type='checkbox' spellcheck='false' class='hidden'>
                    </label>
                </div>
            </div>
        </div>
